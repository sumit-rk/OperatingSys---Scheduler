...
void arng_burst_sjf(int n,int procs[][6])
{
    int i,j,k;
    for(i=0; i<n; i++)
    {
...
void arng_burst_lbt(int n,int procs[][6])
{
    int i,j,k;
    for(i=0; i<n; i++)
    {
...
int avg_time_sjf(int n,int procs[][6])
{
  double total_wtime =0.0;
  double total_ta_time =0.0;
  int j;
  arng_burst_sjf(n,procs);
  wait_time(n,procs);
...
int avg_time_lbt(int n,int procs[][6])
{
  double total_wtime =0.0;
  double total_ta_time =0.0;
  int j;
  arng_burst_lbt(n,procs);
  wait_time(n,procs);
  turnaround_time(n,procs);
...
int main()
{
  ...
  printf("\nProcesses after Arranging for Processes with Shortest Job First  : ");
  arng_burst_sjf(n,procs);
  printf("\nProcess ID\tArrival Time\tBurst Time");
  for(j=0; j<n; j++)
  {
  ...
  printf("\n");
  avg_time_sjf(n,procs);
  printf("\n");
  printf("\n");

  printf("\nProcesses after Arranging for Processes with largest burst time : ");
  arng_burst_lbt(n,procs);
  printf("\nProcess ID\tArrival Time\tBurst Time");
  for(j=0; j<n; j++)
  {
